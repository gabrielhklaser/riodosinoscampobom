...
export interface StationSnap {
  ...
  pEfetiva: number;
  idwWeight: number;
  /** fonte da chuva: 'ANA' (medido) ou 'Open-Meteo' (modelo) */
  rainSource: 'ANA' | 'Open-Meteo';
}
...
/**
 * DH_PER_MM — conversão de chuva efetiva em acréscimo de cota (m/mm).
 * Calibrado no evento 2024: 102,5 mm / 48 h saturado → +6,26 m em ~6 dias.
 */
const DH_PER_MM = 0.0135;
...
function parseAnaLevels(xml: string, code: string): { ts: number; h: number }[] {
  const out: { ts: number; h: number }[] = [];
  ...
  while ((m = re.exec(xml))) {
    ...
    if (!isNaN(d.getTime()) && Number.isFinite(h) && h > 0) out.push({ ts: d.getTime(), h });
  }
...
async function fetchAnaStation(code: string, days: number): Promise<{ ts: number; h: number }[]> {
  const now = new Date();
  ...
  for (const u of proxies) {
    try { const xml = await fetchText(u); const s = parseAnaLevels(xml, code); if (s.length) return s; }
    catch { /* next */ }
  }
  return [];
}
...
async function fetchRainPack(now: number): Promise<RainPack> {
  ...
  const [hist, ecm, gfs, ...anaResults] = await Promise.all([
    ...
    ...anaStations.map((s) => fetchAnaRainHourly(s.anaCode, 14).catch(() => [] as number[])),
  ]);
  ...
  anaResults.forEach((hourly: number[], i: number) => {
    if (anaStations[i] && hourly.length > 0) past[anaStations[i].id] = hourly;
  });
...
/**
 * Busca chuva da ANA (15 min) e agrega em totais horários.
 * Retorna array onde índice 0 = hora mais antiga.
 */
async function fetchAnaRainHourly(code: string, days: number): Promise<number[]> {
  const series = await fetchAnaStation(code, days);
  if (!series.length) return [];

  // parse rain from the raw XML (3rd numeric column)
  const now = new Date();
  const start = new Date(now.getTime() - days * 86400000);
  const end = new Date(now.getTime() + 86400000);
  const raw = `${ANA}?codEstacao=${code}&dataInicio=${brDate(start)}&dataFim=${brDate(end)}`;
  ...
  for (const u of proxies) {
    try {
      ...
      const readings: { ts: number; rain: number }[] = [];
      const re = new RegExp(
      ...
      while ((m = re.exec(xml))) {
        ...
        if (!isNaN(d.getTime()) && Number.isFinite(rain)) {
          readings.push({ ts: d.getTime(), rain });
        }
      }
      if (!readings.length) continue;

      // agregar 15 min → 1 hora
      readings.sort((a, b) => a.ts - b.ts);
      const firstHour = Math.floor(readings[0].ts / 3600000);
      const lastHour = Math.floor(readings[readings.length - 1].ts / 3600000);
      const hourly: number[] = new Array(lastHour - firstHour + 1).fill(0);
      for (const r of readings) {
        const idx = Math.floor(r.ts / 3600000) - firstHour;
        if (idx >= 0 && idx < hourly.length) hourly[idx] += r.rain;
      }
      return hourly.map((v) => +v.toFixed(1));
    } catch {
      continue;
    }
  }
...
/** DH_PER_MM diferenciado por sub-bacia — baixo Sinos (urbano) responde ~2× mais rápido. */
 ...
 *     + Σ_sub[ chuva_efetiva(t - lag_sub) × DH_PER_MM × frac_area ]
 *     + escoamento_base(1 h)
 *     + remanso(Guaíba)  [constante no horizonte]
 *
 * A chuva da hora `t` chega a Campo Bom na hora `t + lag_sub` — isso
 * é a propagação simplificada com defasagem fixa por sub-bacia.
 */
/**
 * PROPAGAÇÃO HORA A HORA.
 *
 * `rainTimeline` é o array contínuo passado+futuro onde:
 *   índice 0  = 24 h atrás de agora
 *   índice 24 = agora (t=0)
 *   índice 25 = +1 h no futuro
 *   ...
 *
 * A chuva que caiu na hora T do timeline chega a Campo Bom
 * na hora T + lag_sub. Isso significa que a chuva das últimas
 * horas a montante (que ainda está "em trânsito") entra na
 * projeção — era o bug que achatava a curva após 24 h.
 */
/**
...
function propagateCurve(
  ...
  for (let t = 1; t <= maxH; t++) {
    ...
    //    e com DH_PER_MM diferenciado (baixo Sinos responde mais rápido)
    let rainContrib = 0;
    for (const sub of subbasins) {
      ...
      if (tlIdx >= 0 && tlIdx < rainTimeline.length) {
        ...
        if (rawMm > 0) {
          ...
          rainContrib += pEff * (DH_SUB[sub] ?? DH_PER_MM) * (AREA_FRAC[sub] ?? 0.33);
        }
      }
    }
    ...
    // 4. Recessão natural — quando não há forçante, o rio recede ~0,3 cm/h
    const forcing = Math.abs(iner) + rainContrib;
    const recession = forcing < 0.0005 && h > 3.0 ? -0.003 : 0;
...
export async function runIphModel(campoBom: { ts: number; h: number }[]): Promise<IphOutput> {
  ...
  const [taquara, guaibaRaw, rain] = await Promise.all([
    fetchAnaStation('87376000', 3),
    fetchAnaStation('87450020', 3),
    fetchRainPack(now),
  ]);
  ...
  const stations: StationSnap[] = IPH_STATIONS.map((st, i) => {
    ...
    const snap: StationSnap = {
      ...
      idwWeight: idwW[i],
      rainSource: (st.anaRain && hourly.length > 0) ? 'ANA' as const : 'Open-Meteo' as const,
    };
