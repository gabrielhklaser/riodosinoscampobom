    ...
    <section className={`p-5 sm:p-6 ${CARD}`}>
      ...
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3.5">
        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-400" />
          ...
          <strong className="font-bold">Módulo experimental.</strong> Modelo estatístico simplificado baseado
          na metodologia MGB / Saint-Venant inercial. Não substitui modelos hidrodinâmicos operacionais nem os
          boletins da Defesa Civil. Limiares deste boletim (4,50 / 5,20 / 6,00 m) são os operacionais
          municipais — distintos das cotas oficiais do SGB usadas no restante do painel.
        </p>
      </div>
      ...
      {out && (
        <>
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-500">
                    ...
                    {' · '}
                    <span className={s.rainSource === 'ANA' ? 'text-blue-300' : 'text-violet-300'}>
                      {s.rainSource === 'ANA' ? '📡 ANA medido' : '🌐 Open-Meteo'}
                    </span>
                  </p>
          ...
          <p className="mt-4 border-t border-slate-800 pt-4 text-[10px] leading-relaxed text-slate-500">
            ...
            baseada na metodologia MGB / Saint-Venant inercial. H<sub>CB</sub>(t+1) = H<sub>CB</sub>(t) +
            inércia(dH/dt) + Σ<sub>sub</sub> chuva_efetiva(t−lag) + escoam. base + remanso(Guaíba).
